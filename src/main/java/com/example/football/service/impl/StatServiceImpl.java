...
public class StatServiceImpl implements StatService {
    ...
    public String importStats() throws JAXBException, FileNotFoundException {
        ...
        StatSeedRootDto statSeedRootDto = xmlParser.fromFile(STAT_FILE_PATH, StatSeedRootDto.class);

        statSeedRootDto.getStats().stream()
                .filter(statSeedDto -> {
                    boolean isValid = validationUtil.isValid(statSeedDto);
                    if (isValid){
                        sb.append(String.format("Successfully imported Stat %.2f - %.2f - %.2f", statSeedDto.getShooting(),
                                statSeedDto.getPassing(), statSeedDto.getEndurance())).append(System.lineSeparator());
                    } else {
